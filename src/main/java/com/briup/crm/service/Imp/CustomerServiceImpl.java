...
public class CustomerServiceImpl implements CustomerService {
	...
	// 获得总人数
	public float getPeopleTotal() {
		CstCustomerExample example = new CstCustomerExample();
		List<CstCustomer> custlist = customerMapper.selectByExample(example);
		float total = custlist.size();
		return total;
	}
	
	// 获得每个区域的人数
	public float getRegionPeopleTotal(String region) {
		CstCustomerExample example = new CstCustomerExample();
		example.createCriteria().andCustRegionEqualTo(region);
		List<CstCustomer> custlist = customerMapper.selectByExample(example);
		float sum = custlist.size();
		return sum;
	}
	
	// 获得每个区域人数的百分比
	@Override
	public float getRegionPeoplePercent(String region) {
		float percent = getRegionPeopleTotal(region)/getPeopleTotal();
		return percent;
	}
